...
double PhaseSpace2to2tauyz::weightGammaPDFApprox(){
  ...
  // Reject non-physical hadronic fractions before evaluating PDFs.
  if (x1GammaHadr > 0.
      && (!isfinite(x1Hadr) || x1Hadr <= 0. || x1Hadr >= 1.))
    return 0.;

  if (x2GammaHadr > 0.
      && (!isfinite(x2Hadr) || x2Hadr <= 0. || x2Hadr >= 1.))
    return 0.;
  ...
  if (!isfinite(sigmaOver) || sigmaOver < TINY) return 0.;
  if (!isfinite(sigmaCorr) || sigmaCorr < 0.) return 0.;

  double wt = sigmaCorr / sigmaOver;

  return isfinite(wt) ? wt : 0.;
